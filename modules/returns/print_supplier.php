<?php
/**
 * Print Supplier Return Receipt - Auto Page Break
...
requireAnyPermission(['returns.view', 'returns.create_supplier']);

$id = $_GET['id'] ?? 0;
$returnUrl = hasPermission('returns.view') ? 'index.php' : (hasPermission('returns.create_supplier') ? 'create_supplier.php' : '../../index.php');

$return = getRow(
    "SELECT r.*, i.invoice_number as original_invoice, s.name as supplier_name_db, s.phone as supplier_phone
     FROM returns r
...
// Supplier info
$supplierName = $return['supplier_name_db'] ?? $return['supplier_name'] ?? '-';
$supplierPhone = $return['supplier_phone'] ?? '-';
...
$pageTitle = 'مرتجع للمورد ' . $return['return_number'];
?>
<!DOCTYPE html>
...
<body>
    ...
    <div class="print-area">
                ...
                <tr>
                    <td colspan="7" class="header-cell">
                        ...
                        <div class="invoice-type">
                            <span>📦 مرتجع للمورد - <?php echo $return['return_number']; ?></span>
        ...
        <div class="summary">
            ...
            <div class="summary-row"><span>استردينا كاش:</span><span><?php echo number_format($cashRefund, 2); ?> <?php echo $currency; ?></span></div>
            <?php elseif ($refundMethod == 'deducted'): ?>
            <div class="summary-row"><span>تم خصمه من حسابنا:</span><span><?php echo number_format($deductedFromBalance, 2); ?> <?php echo $currency; ?></span></div>
        ...
        <div class="refund-box <?php echo $refundMethod; ?>">
            <?php if ($refundMethod == 'cash'): ?>
                💵 تم استرداد <?php echo number_format($cashRefund, 2); ?> <?php echo $currency; ?> كاش من المورد
            <?php elseif ($refundMethod == 'deducted'): ?>
                📝 تم خصم <?php echo number_format($deductedFromBalance, 2); ?> <?php echo $currency; ?> من حسابنا لدى المورد
            <?php elseif ($refundMethod == 'mixed'): ?>
                📝💵 تم خصم <?php echo number_format($deductedFromBalance, 2); ?> من الحساب + استرداد <?php echo number_format($cashRefund, 2); ?> كاش
            <?php endif; ?>
        </div>
